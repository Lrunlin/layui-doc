        ...
        if(9 === ++icon) {
          layer.confirm('怎么样，是否很喜欢该皮肤，去下载？', {
            ...
            layero.find('.layui-layer-btn0').attr({
              href: 'http://layui.blogweb.cn/layer/skin.html',
              target: '_blank'
            });
            layer.close(index);
          });
  ...
  switch(index){
    ...
    case 22:
      ...
      var index = layer.open({
        type: 2,
        content: 'http://layui.blogweb.cn/jump/alyhot/',
        area: ['320px', '195px'],
        maxmin: true
      });
      layer.full(index);
    ...
    layer.alert('', {
      ...
      ,success: function(layero, index){
        ...
        btn.find('.layui-layer-btn0').attr({
          href: 'http://layui.blogweb.cn/jump/alyhot/'
          ,target: '_blank'
        });
      }
      ,end: function(){
  ...
  if(!notLayuiAdmin && false){
    layer.open({
      ...
      ,content: ['<div style="padding: 15px; text-align: center; background-color: #e2e2e2;">'
        ,'<a href="http://layui.blogweb.cn/admin/std/dist/views/" target="_blank"><img src="/upload/2018_5/168_1527691799254_76462.jpg" alt="layuiAdmin" style="width: 100%; height:149.78px;"></a>'
      ,'</div>'].join('')
      ,success: function(layero, index){
        var btn = layero.find('.layui-layer-btn');
        btn.find('.layui-layer-btn0').attr({
          href: 'http://layui.blogweb.cn/admin/std/dist/views/'
          ,target: '_blank'
        });
